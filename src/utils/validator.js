...
exports.depositValidator = [
	check("cardNumber")
		.exists()
		.withMessage("Vui lòng nhập số thẻ")
		.notEmpty()
		.withMessage("Số thẻ không được để trống")
		.isLength({ min: 6, max: 6 })
		.withMessage("Số thẻ phải có 6 chữ số")
		.isIn(["111111", "222222", "333333"])
		.withMessage("Thẻ này không được hỗ trợ"),
...
exports.withdrawValidator = [
	check("cardNumber")
		...
		.withMessage("Số thẻ phải có 6 chữ số")
		.isIn(["111111", "222222", "333333"])
		.withMessage("Thông tin thẻ không hợp lệ")
		.not()
		.isIn(["222222", "333333"])
		.withMessage("Thẻ này không được hỗ trợ để rút tiền"),
	...
	check("price")
		.exists()
		.withMessage("Vui lòng nhập số tiền rút")
		.notEmpty()
		.withMessage("Vui lòng nhập số tiền rút")
		.isNumeric()
		.withMessage("Số tiền rút không hợp lệ")
		.isInt({ min: 50000 })
		.withMessage("Số tiền rút tối thiểu là 50,000 vnđ")
		.isDivisibleBy(50000)
		.withMessage("Số tiền rút phải là bội số của 50,000 đồng"),

	check("message")
		.exists()
		.withMessage("Vui lòng nhập lời nhắn")
		.notEmpty()
		.withMessage("Vui lòng nhập lời nhắn")
		.isLength({ min: 1, max: 100 })
		.withMessage("Lời nhắn phải có từ 1 đến 100 ký tự"),
];
...
exports.transferValidator = [
	check("receiverPhone")
		...
		.custom((value, { req }) => {
			return Account.findOne({ phone: value, role: 0 }).then((account) => {
				if (account) {
					if (account.status === 0) {
						// Tài khoản chưa kích hoạt cmnd
						throw new Error(
							"Tài khoản người nhận chưa đươc kích hoạt nên không thể nhận tiền. Vui lòng thử lại sau!"
						);
					} else if (account.status === 2) {
						// Tài khoản bị vô hiệu hóa
						throw new Error(
							"Tài khoản người nhận đã bị vô hiệu hóa nên không thể nhận tiền. Vui lòng thử lại sau!"
						);
					} else {
						return true;
					}
		...
		.custom((value, { req }) => {
			...
			throw new Error(
				"Số điện thoại người nhận không được trùng với số điện thoại của bạn"
			);
		}),

	check("price")
		.exists()
		.withMessage("Vui lòng nhập số tiền chuyển")
		.notEmpty()
		.withMessage("Vui lòng nhập số tiền chuyển")
		.isNumeric()
		.withMessage("Số tiền chuyển không hợp lệ")
		.isInt({ min: 1000 })
		.withMessage("Số tiền chuyển tối thiểu là 1000 vnđ và phải số nguyên")
	...
	check("isFeeForSender")
		.exists()
		.withMessage("Vui lòng chọn người trả phí giao dịch")
		.notEmpty()
		.withMessage("Vui lòng chọn người trả phí giao dịch")
		.isIn([0, 1])
		.withMessage("Người trả phí giao dịch không hợp lệ")
			...
			if (req.body.price) {
				...
				if (req.body.isFeeForSender == 1) {
					if (Number(account.balance) < Number(req.body.price) * 1.05) {
						throw new Error(
							"Số dư của bạn không đủ để trả phí giao dịch"
						);
					}
				}
			}
